...
for i in range(num_datapoints):
                ...
                get_epsilon = lambda it: return 1 - it*((1 - final_eps)/eps_iterations) if it < eps_iterations else final_eps
                episode_durations, rewards, disc_rewards, losses, trajectories = run_episodes(train_QNet_true_gradient, model, memory,
                                                                            env, num_episodes, batch_size, discount_factor, learn_rate, 
                                                                            get_epsilon=get_epsilon, use_target_qnet=use_target_qnet, 
                                                                            render=render)
                results.append((
                    rewards,
                    testing_seed,
