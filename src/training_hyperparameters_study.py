...
def build_plot(env_name, selection_conditions: Dict =None, w=0.5):
    ...
    for eps_iterations, group in statistics.groupby("eps_iterations"):
        ...
        plt.fill_between(chunks, returns_mean - w*returns_std, returns_mean + w*returns_std, alpha=0.2) #, label=eps_iterations)
    
    plt.legend(title="#Chunks")
    plt.title("Training Hyperparameters Study")
    plt.xlabel("Number of chunks in training")
    plt.ylabel("Training Length")
    plt.show()


build_plot(env_name, {"demonstration_value": 100})
